            ...
            <div className="-space-y-1">
              {titles[index] && (
                <div className="flex items-start gap-5">
                  <div className="bg-brand-white rounded-lg p-4 mt-1">
                    {index === 0 ?
                      <Store size={34} className="text-brand-pink" /> :
                      <Building size={34} className="text-brand-pink" />
                    }
                  </div>
                  <h3 className="text-brand-white font-bold text-xl">{titles[index]}</h3>
                </div>
              )}
              {addresses[index] && (
                <div className="flex items-start gap-5">
                  <div className="bg-transparent rounded-lg p-4 w-[70px] mt-1"></div>
                  <p className="text-brand-white font-medium text-lg">{addresses[index]}</p>
                </div>
              )}
            </div>
