export const handleAmenitiesError = (error, context = '') => {
  if (error.response) {
    ...
    switch (status) {
      case 404:
        return new Error('Diferencial(is) não encontrado(s).')

      case 400:
        return new Error(
          backendMessage
            ? `Erro no(s) diferencial(is): ${backendMessage}`
            : 'Verifique o(s) dado(s) do(s) diferencial(is) informado(s).'
        )

      case 403:
        return new Error(
          'Você não tem permissão para alterar este(s) diferencial(is) informado(s).'
        )

      case 409:
        return new Error('Este(s) diferencial(is) já existe(m).')

      default:
        return new Error(
          backendMessage ||
            `Erro ao processar diferencial(is)${
              context ? ` (${context})` : ''
            }.`
        )
    }
    ...
    return new Error(
      'Erro(s) de conexão ao acessar o(s) diferencial(is). Verifique sua internet e tente novamente. Caso o(s) erro(s) persista(m), contate o suporte.'
    )
  }

  return new Error(
    error.message || 'Erro(s) inesperado(s) no(s) diferencial(is).'
  )
}
