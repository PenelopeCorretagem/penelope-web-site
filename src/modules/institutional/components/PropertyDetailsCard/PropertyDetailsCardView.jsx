...
export function PropertyDetailsCardView({
  ...
  return (
    <div className={`flex flex-col max-w-sm ${hasHoverEffect ? 'transition-transform hover:scale-105' : ''}`}>
      ...
      <div
        className={`bg-default-light-secondary p-card md:p-card-md gap-card md:gap-card-md flex w-full flex-col items-start ${hasImage ? 'rounded-b-sm' : 'rounded-sm'} ${hasShadow ? 'drop-shadow-md' : ''}`}
      >
