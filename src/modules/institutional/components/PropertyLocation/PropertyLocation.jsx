...
export function PropertyLocation({ locations, addresses = [], titles = [] }) {
  return (
    <>
      <HeadingView level={3} className="text-default-light mb-10">
        Localização
      </HeadingView>
          ...
          <div key={loc.type || index} className="flex flex-col h-full">
            <div className="flex items-start gap-3">
              <div className="bg-default-light rounded-lg p-4">
                {index === 0 ?
                  <Store size={34} className="text-distac-primary" /> :
                  <Building size={34} className="text-distac-primary" />
                }
              </div>
              <div>
                {titles[index] && (
            ...
            <TextView className="text-default-light text-base font-semibold mb-4">
              Veja como chegar
            </TextView>
            <div className="bg-distac-primary p-6 rounded-lg min-h-[440px] flex flex-col items-center justify-center">
              <HeadingView level={5} className="font-semibold uppercase text-sm">
                {loc.type}
              </HeadingView>
              <TextView className="mt-1 text-default-light">
                {loc.address}
              </TextView>
              <button className="mt-4 bg-white text-distac-primary px-4 py-2 rounded-lg font-semibold hover:bg-gray-100">
                Exibir Mapa
              </button>
            </div>
          </div>
